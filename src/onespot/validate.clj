...
(defn validate-entity
  ...
  (case kind
    ...
    :attr   (let [attr-entity (os/attr-entity entity)
    ...
    ;; passed.
    :rec (or (nil-check   entity value path)
             (empty-check entity value path)
             (->> (os/rec-attrs entity)
                  (map (fn [{attr-id :entity-id :as attr}]
                         (let [attr-value (get value attr-id)
                               attr-path  (conj path attr-id)
                               required?  (os/required? entity attr)]
    ...
    ;; path.
    :series (or (nil-check   entity value path)
                (empty-check entity value path)
                (let [series-entity (os/series-entity entity)]
                  (->> (map (fn [idx value]
